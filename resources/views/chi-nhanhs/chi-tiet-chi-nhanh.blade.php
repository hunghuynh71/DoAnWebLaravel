      ...
      <div class="card-body">
        <div class="row">
          <div class="col-12 col-md-12 col-lg-12 order-12 order-md-12">
            <h1 class="text-primary text-center">{{$chi_nhanh->ten_cn}}</h1>
            <p>Địa chỉ: {{$chi_nhanh->dia_chi}}</p>
            <p>SĐT: {{$chi_nhanh->sdt}}</p>
            <p>Hình ảnh: {{$chi_nhanh->hinh_anh}}</p>
            <div class="text-center mt-5 mb-3">
              <a href="{{route('chi-nhanh.editChiNhanh',$chi_nhanh->id)}}" class="btn btn-sm btn-primary">Chỉnh sửa</a>
              <a href="{{route('chi-nhanh.getChiNhanhs')}}" class="btn btn-sm btn-warning">Thoát</a>
            </div>
          </div>
        </div>
      </div>
